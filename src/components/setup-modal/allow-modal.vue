<template>
  <div>
    <Card>
      <Modal v-model="showModal" :title="formValidate.id? $t('setUpPage.editRole') : $t('setUpPage.addRole')" :mask-closable="false">
        <div slot="close">
        ...
        <Tree :data="roleTree" @on-check-change="changeTree" ref="tree" show-checkbox></Tree>
        <div slot="footer">
...
<script>
import { editRoleInfo, addRoleInfo, getRoleTreeInfo, getOneRole } from '@/api/data'
...
export default {
  ...
  data () {
    return {
      ...
      ruleValidate: {
        name: [ { required: true, message: this.$t('setUpPage.charactorTip'), trigger: 'blur' } ],
        description: [ { required: false, message: this.$t('setUpPage.descriptionTip'), trigger: 'change' } ]
      }
    }
  },
  methods: {
