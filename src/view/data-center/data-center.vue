<template>
  <div>
    <Row :gutter="20">
      <i-col :xs="12" :md="8" :lg="6" v-for="(infor, i) in registerData" :key="`infor-${i}`" style="margin-bottom: 15px;height: 120px;padding-bottom: 10px;">
    ...
    <Row :gutter="20">
      <i-col :md="24" :lg="24" style="margin-bottom: 20px;">
        <Card shadow>
          <line-chart style="height: 310px" :dateArray="dateArray" :regisiterArray="regisiterArray"/>
        </Card>
      </i-col>
    </Row>
    <Row :gutter="20">
...
import { getDataCenterInfo } from '@/api/data'
export default {
  name: 'data_center',
  components: {
    InforCard,
    ...
    getPageInfo () {
      getDataCenterInfo().then(res => {
        if ( res.data.success ) {
          ...
          let today = new Date(parseInt(data.time))
          data.registerLastWeek.forEach((item, index) => {
            let thisDay = new Date(today.getTime() - 24*60*60*1000*(index)).toLocaleString().substring(0, 10)
            this.dateArray.push(thisDay)
          })
          this.dateArray.reverse()

          this.regisiterArray = data.registerLastWeek
          this.regisiterArray.reverse()
          
          
          this.registerData = [
            { title: this.$t('dataCenter.registerTotal'), icon: 'md-person-add', count: data.registerTotalCount, color: '#2d8cf0' },
